...
class ConverseAgent:
    ...
    def _get_converse_response(self):
        ...
        print(Colors.info(json.dumps(payload, indent=2, ensure_ascii=False)))
        # API 호출
        response = self.client.converse(**payload)

        # 응답 로깅
        print(Colors.warning("\n=== Bedrock Converse Response ==="))
        print(Colors.info(json.dumps({
    ...
    async def _handle_response(self, response):
        ...
        print(Colors.warning("\n=== Thinking.. Bedrock Handle Response ==="))
        print(Colors.success("stop reason >> " + stop_reason))
        print(Colors.info(response['output']['message']['content']))
